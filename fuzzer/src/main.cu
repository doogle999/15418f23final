...
void runInstruction(State& state, std::uint32_t inst, uint8_t* memory) {
    ...
    switch (opcode) {
        ...
        case 0x63: // beq, bne, blt, bge, bltu, bgeu
        {
            ...
            std::uint32_t imm = ((inst & (1u << 31)) >> 19) | ((inst & 0x7e000000) >> 20) | (rd & 0x1e) | ((rd & 0x1) << 11);
            if (inst & (1u << 31)) {
                imm |= 0xffffe000;
            }
        ...
        case 0x33: // add, sub, sll, slt, sltu, xor, srl, sra, or, and
        {
            ...
            switch ((inst >> 12) & 0x7) {
                ...
                case 0x3: // sltu
                {
                    state.x[rd] = (static_cast<std::uint32_t>(state.x[rs1]) < static_cast<std::uint32_t>(state.x[rs2])) ? 1 : 0;
                    break;
                }
                case 0x4: // xor
                {
...
int main(int argc, char** argv) {
    ...
    std::uint32_t memorySize = 0xff;
    memory = static_cast<uint8_t*>(malloc(memorySize + programSize));
    if (!memory) {
    ...
    const auto DONE_ADDRESS = 0xfffffff0u;
    ...
    while (true) {
        std::uint32_t inst = *reinterpret_cast<std::uint32_t*>(program + state.pc);
    ...
    for (std::uint32_t i = 0; i < memorySize; i += 4) {
